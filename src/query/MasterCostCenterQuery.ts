...
import {
  ICostCenterCollaction,
  IFormCostCenter,
} from "@/types/MasterCostCenter";

//IMPORTAN VARIABLE
const queryNameKey = "collactionDataMasterCostCenter";
const queryNameTree = "collactionDataCostCenterTreeInput";
const endPointUrl = "/master/cost-center";
const endPointMove = "/master/cost-center/move";
...
//custom hook fetch data collaction
export const useQueryDataCostCenter = ({ token }: IProps) => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get<ICostCenterCollaction>(endPointUrl, {
...
export const useMutationTree = ({ token }: IProps) => {
  ...
  const createNewData = useMutation(async (data: IFormCostCenter) => {
    const formData = new FormData();
  ...
  const handleCreateNew = async (data: IFormCostCenter) => {
  ...
  const updateDataKategori = useMutation(async (data: IFormCostCenter) => {
    const formData = new FormData();
    formData.append("name", data.name as string);
    formData.append("description", data.description as string);
    if (data.parentId) {
  ...
  //handle update data
  const handleUpdateData = async (data: IFormCostCenter) => {
  ...
  const deleteData = useMutation(async (data: IFormCostCenter) => {
    const formData = new FormData();
  ...
  const handleDeleteData = async (data: IFormCostCenter) => {
...
export const useQueryListDataTeeInput = ({ token }: IProps) => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get<ICostCenterCollaction>(
        "/master/cost-center/get-list-data-for-input",
        {
          headers: { Authorization: "Bearer " + token },
        }
      );
