...
const FormCreate = ({ onSubmit, onCancel }: IPropsForm) => {
  ...
  //react query
  const { handleCreate } = useMutationDataMasterCurrency({
  ...
  return (
    <Spin spinning={isLoading}>
      <Form
        ...
        style={{ maxWidth: 600 }}
      >
        <FormItem control={control} label="Name" name="iso">
          <Input />
        </FormItem>

        <FormItem control={control} label="Kode" name="symbol">
          <Input />
        </FormItem>
...
const BtnFormCreate = () => {
  ...
  return (
    <>
      ...
      >
        <BsPlusLg /> Tambah Mata Uang
      </button>
      <Modal
        title="Tambah Data User Aset"
        open={isOpen}
        footer={null}
        destroyOnClose
